/* Klasse ImageArrayOwn: Transformiert Bitmaps in Array-Formate und zurück, ohne den Inhalt zu verändern.
...
namespace ImageProcessing {
  internal class ImageArrayOwn {

    #region 1. Image -> 3D-Array -> Image   
    /// <summary> GENERISCHE METHODE <T> byte, float, double</T>
    /// 1.1 Konvertiert eine Bitmap in eine dreidimensionales 
    ...
    /// <summary> GENERISCHE METHODE <T> byte, float, double</T>
    /// 1.2: Wandelt eine dreidimensionales Array in ein Bitmap um.
    ...
    /// <param name="array3D"></param>  3D-Array
    ...
    #region 4. Histogramm-Feld -> 2D-Feld --> Histogramm-Feld

    /// <summary>
    /// Konvertiert RGB-Histogramm [Intensität].r,g,b in ein zweidimensionales Feld [canal,intensity]
    /// </summary>
    /// <param name="histogramm"></param> RGB-Histogramm
    /// <returns></returns> Histogramm als zweidimensionales Feld [canal, intensity]
    public int[,] toFeld2D(HistogramOwn.Histogram[] histogramm) {
      int[,] feld2D = new int[3, histogramm.GetLength(0)];
      // Übeträgt die Strukturvariablen r,g,b in Kanalindizierte Elemente.
      // I - intensity
      for (int I = 0; I < histogramm.GetLength(0); I++) {
        feld2D[0, I] = histogramm[I].r;
        feld2D[1, I] = histogramm[I].g;
        feld2D[2, I] = histogramm[I].b;
      }
      return feld2D;
    }

    /// <summary>
    /// Konvertiert zweidimensionales Feld [canal,intensity] in ein RGB-Histogramm [intensity].r,g,b.
    /// </summary>
    /// <param name="feld2D"></param> zweidimensionales Feld [canal, intensity]
    /// <returns></returns>RGB-Histogramm
    public HistogramOwn.Histogram[] toRGB(int[,] feld2D) {
      HistogramOwn.Histogram[] histogramm = new HistogramOwn.Histogram[feld2D.GetLength(1)];
      // Übeträgt die Strukturvariablen r,g,b in Kanalindizierte Elemente.
      // I - intensity
      for (int I = 0; I < feld2D.GetLength(1); I++) {
        histogramm[I].r = feld2D[0, I];
        histogramm[I].g = feld2D[1, I];
        histogramm[I].b = feld2D[2, I];
      }
      return histogramm;
    }
    #endregion
